...
\title{Steam Games Dataset: Procesamiento, Análisis Descriptivo, Asociación, Agrupamiento y Clasificación
}
...
\begin{abstract}
Steam es una de las plataformas de distribución digital de videojuegos más importantes del mundo y genera grandes volúmenes de información sobre comportamiento de usuarios, características de los juegos y métricas de éxito comercial. En este trabajo se realizó un análisis exploratorio y un proceso de preprocesamiento sobre el Steam Games Dataset, compuesto por más de 122 000 títulos. Inicialmente se caracterizó estadísticamente el conjunto de datos, identificando distribuciones altamente asimétricas, una fuerte presencia de valores atípicos y diferencias marcadas entre juegos de nicho y títulos de gran popularidad. Posteriormente se aplicaron técnicas de limpieza, tratamiento de valores faltantes, ingeniería de atributos y codificación de variables categóricas. Sobre los datos procesados se desarrollaron análisis de reglas de asociación, agrupamiento y clasificación supervisada con el fin de identificar patrones relacionados con el éxito de los videojuegos. Los resultados evidencian asociaciones significativas entre determinadas características de los juegos y métricas como ventas, volumen de reseñas y actividad de usuarios, además de permitir la identificación de perfiles comerciales diferenciados dentro de la plataforma. Asimismo, los modelos de clasificación demostraron la importancia de estrategias adecuadas para el manejo del desbalance de clases. Este trabajo establece una base para el desarrollo de modelos predictivos y herramientas de apoyo a la toma de decisiones dentro de la industria de los videojuegos.
\end{abstract}

\begin{IEEEkeywords}
  Steam, videojuegos, minería de datos, aprendizaje automático, reglas de asociación, clustering, clasificación.
\end{IEEEkeywords}
...
Se utilizó el ``Steam Games Dataset'' \cite{martin_bustos_roman_2022}, obtenido a partir de la API oficial de Steam. Este \textit{dataset} cuenta con 122\,611 registros, correspondientes cada uno a un videojuego único, y está compuesto por 39 variables originales.
...
\end{tabular}
\caption{Reseñas y tasas mínimas para categorizar las reseñas.}
\label{table_1}
\end{table}

También se creó una categorización para las ventas, con el atributo "No data" para aquellos juegos sin ventas. En la tabla \ref{table_2} se tienen las categorías para las ventas.
...
\end{tabular}
\caption{Categorías para las ventas.}
\label{table_2}
\end{table}
...
Las tablas desde la \ref{table_3} hasta la \ref{table_12} muestran las 10 mejores reglas generadas por los dos algoritmos para cada consecuente, seguido de las 10 reglas globales más frecuentes. 
...
\caption{Reglas para buenas reseñas con FP-growth con 126 reglas totales}
\label{table_3}
\end{table}
...
\caption{Reglas para buenas reseñas con Apriori con 37 reglas totales}
\label{table_4}
\end{table}
...
\caption{Reglas para buenas ventas con FP-Growth con 4 reglas totales}
\label{table_5}
\end{table}
...
\caption{Reglas para buenas ventas con Apriori con 4 reglas totales}
\label{table_6}
\end{table}
...
\caption{Reglas para reseñas en el percentil 50 con FP-Growth con 192 reglas totales}
\label{table_7}
\end{table}
...
\caption{Reglas para reseñas en el percentil 50 con Apriori con 37 reglas totales}
\label{table_8}
\end{table}
...
\caption{Reglas para reseñas en el percentil 75 con FP-Growth con 87 reglas totales}
\label{table_9}
\end{table}
...
\caption{Reglas para reseñas en el percentil 75 con Apriori con 34 reglas totales}
\label{table_10}
\end{table}
...
\caption{Top 10 reglas con FP-Growth}
\label{table_11}
\end{table}
...
\caption{Top 10 reglas con Apriori}
\label{table_12}
\end{table}
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_11.png}}
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_12.png}}
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_13.png}}
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=\linewidth]{figures/tabla_1.png}}
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_14.png}}
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=\linewidth]{figures/tabla_2.png}}
...
\begin{figure}[htbp]
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_15.png}}
...
En esta etapa se abordó la clasificación supervisada sobre el conjunto de datos completo de juegos de Steam, con el objetivo de predecir si un título alcanzaría una valoración positiva alta a partir de sus atributos estructurales, temporales y de recepción por parte de los usuarios. Para ello, se construyó una variable objetivo binaria a partir de \texttt{review\_score\_cat}, considerando como clase positiva los juegos \textit{Very Positive} y \textit{Overwhelmingly Positive}. Los registros etiquetados como \textit{No reviews} y \textit{Too few reviews} fueron excluidos, puesto que no ofrecen evidencia suficiente para inferir con fiabilidad la calidad percibida del producto.

Sobre este conjunto depurado se efectuó una verificación exhaustiva de las variables candidatas al modelado. Se revisó la integridad de las columnas numéricas, la ausencia de valores faltantes y la correcta naturaleza binaria de las variables derivadas por One-Hot Encoding para géneros y etiquetas. De manera complementaria, se descartaron variables susceptibles de inducir fuga de información, en particular aquellas que derivan de la propia calificación o de estadísticas que la definen de forma casi determinista. Esta depuración fue necesaria para asegurar que los modelos aprendieran patrones explicativos y no reglas indirectas de reconstrucción de la clase objetivo.

La partición de los datos se realizó mediante un esquema holdout estratificado 80/20, con el propósito de conservar la proporción de clases en ambos subconjuntos. Este paso es especialmente relevante cuando se trabaja con clases desbalanceadas, ya que una partición aleatoria no estratificada podría distorsionar el error aparente y comprometer la comparabilidad entre experimentos. Adicionalmente, se aplicó escalamiento estándar únicamente sobre las variables numéricas continuas, preservando intactas las variables binarias. Esta decisión mantuvo la semántica original de las columnas codificadas y evitó introducir transformaciones innecesarias sobre atributos que ya se encontraban en una escala natural de $0$ y $1$.

La comparación de modelos incluyó tres enfoques con propiedades complementarias. El Árbol de Decisión aportó interpretabilidad mediante reglas explícitas; Na"{i}ve Bayes Gaussiano ofreció una referencia probabilística simple y eficiente; y la red neuronal de tipo perceptrón multicapa (MLP) permitió capturar relaciones no lineales entre variables heterogéneas. Dado el desbalance de clases observado en la variable objetivo, la evaluación no se apoyó exclusivamente en la exactitud, sino en métricas más sensibles al desempeño de la clase minoritaria, tales como \textit{precision}, \textit{recall}, \textit{F1-score} y \textit{ROC-AUC}. La validación cruzada estratificada de 10 pliegues complementó el holdout y aportó una estimación más estable del comportamiento de cada clasificador frente a distintas particiones del conjunto de entrenamiento.

\begin{figure}[htbp]
  \centering
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_16.png}}
  \caption{Distribución de la variable objetivo y comparación con clasificadores ingenuos.}
  \label{fig:clasif_dataset_completo_dist}
\end{figure}

\begin{figure}[htbp]
  \centering
  % Espacio reservado para matrices de confusión, curvas ROC y tabla comparativa de métricas.
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_17.png}}
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_18.png}}
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_19.png}}
  \caption{Evaluación comparativa de los clasificadores sobre el conjunto de prueba.}
  \label{fig:clasif_dataset_completo_eval}
\end{figure}

En términos metodológicos, el análisis evidenció que la exactitud por sí sola puede resultar engañosa en escenarios desbalanceados: un clasificador trivial puede alcanzar una puntuación elevada sin identificar adecuadamente la clase de interés. Por ello, se dio prioridad al estudio conjunto de las matrices de confusión, las curvas ROC y las métricas orientadas a la clase positiva, las cuales permiten valorar de forma más realista el poder discriminativo del modelo. En síntesis, este bloque estableció una línea base sólida para la clasificación supervisada del conjunto completo de juegos y proporcionó criterios consistentes para interpretar los experimentos posteriores.

\subsection{Análisis de los juegos de interés}

El segundo experimento se concentró en el subconjunto de juegos de mayor interés analítico, compuesto por títulos con reseñas suficientemente confiables y orientado a distinguir entre \textit{Very Positive} y \textit{Overwhelmingly Positive}. A diferencia del análisis general, en este caso el objetivo fue explorar con mayor detalle los factores que acompañan la excelencia sostenida en la recepción del público, utilizando variables de precio, tiempos de juego, plataformas, géneros y etiquetas de Steam. Esta formulación permite estudiar no solo qué juegos triunfan, sino también qué configuraciones de atributos tienden a separar a los títulos altamente valorados del conjunto de juegos positivamente recibidos.

El procedimiento inició con la construcción del conjunto de características a partir de variables numéricas seleccionadas y codificaciones binarias ya disponibles en el preprocesamiento previo. Se conservaron atributos que describen el comportamiento comercial y de uso del juego, como el precio, el pico de usuarios concurrentes, los tiempos de juego promedio y mediano, además de las banderas binarias de sistema operativo, géneros y etiquetas frecuentes. Para evitar contaminación entre entrenamiento y prueba, se aplicó una partición estratificada y se escaló únicamente el bloque numérico. Esta separación fue necesaria porque el conjunto de atributos combina variables continuas con indicadores binarios dispersos, cuya escala no debe homogeneizarse indiscriminadamente.

\begin{figure}[htbp]
  \centering
  % Espacio reservado para la distribución de clases en el subconjunto de juegos de interés.
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_20.png}}
  \caption{Distribución de clases en los juegos de interés.}
  \label{fig:clasif_interes_dist}
\end{figure}

Debido a que la clase positiva minoritaria representa el caso de mayor interés, se incorporaron estrategias específicas para contrarrestar el desbalance. Entre ellas destacan el uso de pesos de clase en los modelos sensibles a la distribución, la optimización del umbral de decisión a partir de la curva Precision-Recall y, en el caso de la red neuronal, un esquema de submuestreo del conjunto de entrenamiento. Esta combinación de medidas buscó reducir el sesgo hacia la clase mayoritaria y mejorar la recuperación de títulos que realmente alcanzan la categoría de excelencia máxima.

Se compararon varios modelos con distinta naturaleza inductiva: un Random Forest como referencia robusta sobre datos tabulares, un HistGradientBoostingClassifier, una Regresión Logística y una red neuronal MLP. Cada uno de ellos ofrece una perspectiva distinta sobre el problema: los modelos de ensamble explotan la interacción entre variables sin requerir supuestos fuertes, la Regresión Logística funciona como baseline lineal sobre un espacio de alta dimensión y la red neuronal introduce capacidad no lineal adicional. La evaluación se diseñó alrededor de métricas sensibles al desbalance, en especial \textit{F1-score} optimizado y \textit{Precision-Recall AUC}, ya que estos indicadores reflejan mejor la capacidad del modelo para identificar la clase positiva rara sin inflar artificialmente el rendimiento mediante la clase mayoritaria.

\begin{figure}[htbp]
  \centering
  % Espacio reservado para curvas Precision-Recall, umbrales óptimos y matrices de confusión.
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_21.png}}
  \caption{Comparación de modelos mediante curvas Precision-Recall y matrices de confusión.}
  \label{fig:clasif_interes_pr}
\end{figure}

\begin{figure}[htbp]
  \centering
  % Espacio reservado para la comparación entre la red neuronal original y la red optimizada.
  \centerline{\includegraphics[width=\linewidth]{figures/grafica_22.png}}
  \caption{Comparación de la red neuronal original y la versión optimizada.}
  \label{fig:clasif_interes_mlp}
\end{figure}

Los resultados permitieron observar que el ajuste del umbral de decisión tiene un impacto sustantivo en la recuperación de la clase positiva, especialmente en contextos de fuerte asimetría entre clases. En particular, la optimización del umbral mediante la maximización del \textit{F1-score} resulta más informativa que el umbral fijo de $0.5$, porque reduce falsos negativos y mejora la sensibilidad respecto del grupo minoritario, que es precisamente el de mayor interés analítico. Asimismo, la comparación entre modelos mostró que los métodos de ensamble y la red neuronal optimizada ofrecen alternativas complementarias: los primeros aportan robustez y estabilidad, mientras que la red neuronal puede beneficiarse de una estrategia explícita de balanceo y regularización cuando se entrena sobre datos submuestreados.

En conjunto, este análisis proporciona una caracterización más fina de los juegos de interés y de los rasgos que acompañan a las calificaciones más altas dentro del ecosistema de Steam. Además de confirmar la utilidad de las métricas centradas en la clase positiva, el estudio refuerza la idea de que, en problemas desbalanceados, la selección del umbral y la estrategia de balanceo influyen de manera decisiva en la calidad práctica del clasificador.

\section{Conclusión y Trabajos Futuros}
El presente trabajo permitió estudiar de manera integral el ecosistema de videojuegos de Steam mediante la aplicación de técnicas de análisis exploratorio, preprocesamiento, minería de patrones y aprendizaje automático sobre un conjunto de más de 122 000 títulos. Los resultados obtenidos evidencian que el catálogo de Steam presenta una distribución fuertemente desigual, donde una pequeña proporción de juegos concentra gran parte de las ventas, las reseñas y la actividad de usuarios, mientras que la mayoría de los títulos mantienen niveles reducidos de visibilidad e impacto comercial.

La fase de preprocesamiento resultó fundamental para adaptar el conjunto de datos a tareas posteriores de análisis. La eliminación de atributos irrelevantes, el tratamiento de valores faltantes, la creación de variables derivadas y la discretización de métricas clave permitieron transformar información heterogénea en representaciones más adecuadas para los algoritmos de minería de datos. Asimismo, la conservación selectiva de ciertos valores extremos, particularmente aquellos asociados a métricas de popularidad, permitió preservar información esencial sobre los juegos más exitosos de la plataforma.

Los algoritmos de reglas de asociación revelaron patrones consistentes entre determinadas características de los videojuegos y sus indicadores de éxito. En particular, atributos relacionados con experiencias multijugador, cooperación entre jugadores, mundo abierto y ciertas funcionalidades propias de Steam mostraron asociaciones relevantes con mayores niveles de ventas, actividad concurrente y volumen de reseñas. Además, el análisis de los juegos de interés permitió identificar combinaciones de géneros y etiquetas que aparecen frecuentemente en títulos con valoraciones excepcionalmente positivas, aportando evidencia sobre preferencias recurrentes de la comunidad de usuarios.

El análisis de agrupamiento mostró que incluso dentro de los juegos exitosos existen perfiles claramente diferenciados. Se identificaron grupos asociados a títulos premium de alta popularidad, juegos de nicho con elevada retención de usuarios, productos de bajo costo con patrones de uso particulares y casos excepcionales representados por fenómenos masivos como Counter-Strike 2 y Dota 2. La comparación entre K-Means, Agglomerative Clustering y DBSCAN evidenció que los métodos particionales ofrecen una representación más adecuada para este conjunto de datos, debido a la presencia de distribuciones densas y numerosos valores extremos.

Por su parte, los experimentos de clasificación confirmaron que el éxito percibido de un videojuego puede modelarse de forma razonablemente efectiva utilizando atributos estructurales, comerciales y de comportamiento. Los resultados resaltan la importancia de emplear métricas adecuadas para conjuntos desbalanceados y demuestran que técnicas como la optimización de umbrales de decisión, el balanceo de clases y la selección cuidadosa de variables tienen un impacto significativo sobre la capacidad predictiva de los modelos. Además, la comparación entre árboles de decisión, modelos probabilísticos, métodos de ensamble y redes neuronales mostró que distintos enfoques ofrecen ventajas complementarias entre interpretabilidad y capacidad predictiva.

Como trabajo futuro, resulta especialmente interesante incorporar información temporal detallada sobre la evolución de precios, descuentos, actividad de usuarios y volumen de reseñas. Este tipo de información permitiría construir modelos dinámicos capaces de estudiar el ciclo de vida completo de un videojuego y no únicamente una fotografía estática de su estado actual. De igual manera, podría explorarse la aplicación de técnicas avanzadas de balanceo, métodos de ensamble más sofisticados y algoritmos de aprendizaje profundo orientados a mejorar la capacidad de generalización de los modelos predictivos.

Otra línea prometedora consiste en integrar fuentes de información externas, tales como datos provenientes de redes sociales, transmisiones en plataformas de streaming, tendencias de búsqueda o actividad en comunidades de jugadores. La combinación de estas fuentes con las métricas internas de Steam permitiría obtener una visión más completa de los factores que impulsan la popularidad y la recepción de un videojuego.

Finalmente, los resultados obtenidos sientan las bases para el desarrollo de sistemas de recomendación y herramientas de apoyo a la toma de decisiones dirigidas a desarrolladores, estudios independientes y editores. Estos sistemas podrían utilizar los patrones descubiertos para estimar el potencial de éxito de nuevos proyectos, identificar nichos de mercado poco explotados y apoyar estrategias de lanzamiento, monetización y posicionamiento dentro de la plataforma Steam.
